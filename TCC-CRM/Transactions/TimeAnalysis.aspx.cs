...
namespace TCC_CRM.Transactions
{
    public partial class TimeAnalysis : System.Web.UI.Page
    {
        ...
        private void LoadDropdown()
        {
            ...
            ASPxListBox ListBoxCustomer = ((ASPxListBox)ASPxDropDownEdit_Customer.FindControl("ListBoxCustomer"));
            ListBoxCustomer.DataSource = dtLoadList;
            ListBoxCustomer.TextField = "customer_name";
            ListBoxCustomer.ValueField = "customer_id";
        ...
        private void bindgridview()
        {
            ...
            ASPxListBox ListBoxCustomer = ((ASPxListBox)ASPxDropDownEdit_Customer.FindControl("ListBoxCustomer"));
            if (ListBoxCustomer.SelectedItems.Count > 0)
            {
                for (int i = 0; i < ListBoxCustomer.SelectedItems.Count; i++)
                {
                    int strselecteditems = Convert.ToInt32(ListBoxCustomer.SelectedItems[i].Value.ToString());
                    if (!string.IsNullOrEmpty(strCustomerID))
                    {
                        strCustomerID = strCustomerID + "','" + strselecteditems;
                    }
                    else
                    {
                        strCustomerID = strselecteditems.ToString();
                    }
                }
            }
            else
            ...
            DataTable dt = m_BLLCommon.GetCustomerTimeAnalysisList(StartTime, EndTime, strEngineerID, strCustomerID, SessionMgr.DBName);
