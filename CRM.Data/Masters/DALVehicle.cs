...
{
    public class DALVehicle : Base.Data.BaseSql
    {
        public DataTable GetVehicleList(string dbname)
        {
            string strSql = @"select VehicleID,VehicleCode ,VehicleName,VehicleNo,Remarks from engineerVehicle WHERE Active=1";
                ...
                using (MySqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        using (MySqlCommand cmd = conn.CreateCommand())
                        {
                            ...
                            if (br.VehicleID == 0)
                            {
                                cmd.CommandText = @" INSERT INTO engineerVehicle(CompanyID,VehicleCode,  VehicleName,VehicleNo,Remarks, CreatedBy,ModifiedBy,Active)
                                             VALUES (@CompanyID,@VehicleCode, @VehicleName,@VehicleNo,@Remarks, @createdby, @modifiedby,1) ";
                            }
                            else
                            {
                                cmd.CommandText = @" UPDATE engineerVehicle SET VehicleCode = @VehicleCode, VehicleName = @VehicleName,VehicleNo= @VehicleNo,Remarks=@Remarks, ModifiedBy=@modifiedby WHERE  VehicleID = @VehicleID ";
                            ...
                            cmd.Parameters.Add(AddTypedParam("@VehicleCode", MySqlDbType.VarChar, br.VehicleCode));
                            cmd.Parameters.Add(AddTypedParam("@VehicleName", MySqlDbType.VarChar, br.VehicleName));
                            cmd.Parameters.Add(AddTypedParam("@VehicleNo", MySqlDbType.VarChar, br.VehicleNo));
        ...
        public int DeleteVehicle(int VehicleID, int modifiedBy,string dbname)
        {
            ...
            try
            {
                string strSql = " Update engineerVehicle Set Active = 0, ModifiedBy=@modifiedBy, ModifiedDT=Now() Where VehicleID = @VehicleID ";


                MySqlParameter[] parameters = { 
                                                new MySqlParameter("@VehicleID", MySqlDbType.Int64),
